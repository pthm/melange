...
The Rover implementation used a generalized, recursive function. This is fine for simpler models, but as I tried to implement more of the **OpenFGA 1.1 spec**—specifically **wildcards** (for public access) and **usersets** (nested groups)—the generalized logic became incredibly complex and difficult to optimize.

I got "nerd-sniped." I spent the holiday implementing the full spec, but I decided on a different architectural path.

Instead of a generalized function that interprets your model at runtime, I built **Melange**. Melange is a compiler. It reads your OpenFGA schema and **generates specialized PostgreSQL functions** for every relationship you define.

- Instead of a single generic `check_permission('user:1', 'writer', 'repo:5')`, you get a generated `check_repo_writer(1, 5)`.
- Because the SQL is generated for _your specific model_, the database planner can optimize the paths perfectly.
- It supports the advanced 1.1 features like "Tuple-to-Userset" (TTU) patterns and wildcards that the original Rover scripts didn't cover.
...
Melange gives you the sophisticated modeling of OpenFGA with the reliability of a local Postgres function. It’s designed to carry you from your first user until the day you’re actually big enough to need the real Zanzibar/OpenFGA.

Because it's built on OpenFGA's schema you can easily migrate to OpenFGA when you're ready. Melange is open source and available on [GitHub](https://github.com/pthm/melange).