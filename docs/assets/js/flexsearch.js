...
//
// ponytail: fork of hextra v0.12.2 assets/js/flexsearch.js with four deltas,
// each marked `melange:`. Re-apply them when bumping the hextra module.
//   1. strip HTML tags out of heading titles
//   2. dedupe result rows by URL instead of URL+paragraph
//   3. rank /blog/ release notes below /docs/
//   4. rank by title match (page title, then section title), page root above its sections
  ...
  async function preloadIndex() {
    ...
    for (const route in data) {
      ...
      for (const heading in data[route].data) {
        ...
        // melange: heading titles come from Hugo as raw HTML (e.g. `<code>migrate</code>`)
        const title = (text || data[route].title).replace(/<[^>]+>/g, '');
  ...
  function search(e) {
    ...
    for (let i = 0; i < pageResults.length; i++) {
      ...
      for (let j = 0; j < nResults; j++) {
        ...
        // melange: one row per section, not one per matching paragraph
        if (occurred[url]) continue
        occurred[url] = true
        results.push({
    ...
    // melange: does every query word appear in the page title?
    const queryWords = query.toLowerCase().split(/\s+/).filter(Boolean);
    const titleHit = (t) => {
      const title = (t || '').toLowerCase();
      return queryWords.every(w => title.includes(w)) ? 1 : 0;
    };

    const sortedResults = results
      .sort((a, b) => {
        // melange: release notes repeat docs vocabulary; keep them below /docs/.
        const aBlog = a.route.startsWith('/blog/') ? 1 : 0;
        const bBlog = b.route.startsWith('/blog/') ? 1 : 0;
        if (aBlog !== bBlog) return aBlog - bBlog;
        // melange: page named by the query wins, then section named by it.
        // This is what makes `check_permission` land on the SQL API reference.
        const titleRank = (x) => titleHit(x._page_title) ? 2 : (titleHit(x._section_title) ? 1 : 0);
        const aTitle = titleRank(a);
        const bTitle = titleRank(b);
        if (aTitle !== bTitle) return bTitle - aTitle;
        // Sort by number of matches in the title.
        if (a._page_rk === b._page_rk) {
          // melange: the page itself before any of its sections.
          const aHash = a.route.includes('#') ? 1 : 0;
          const bHash = b.route.includes('#') ? 1 : 0;
