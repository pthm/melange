...
{{- range $.ComplexExcludedRelations }}
          AND check_permission_internal(t.subject_type, substring(t.subject_id from 1 for position('#' in t.subject_id) - 1) || '#' || v_filter_relation, '{{.}}', '{{$.ObjectType}}', p_object_id, ARRAY[]::TEXT[]) = 0
{{- end }}
{{- end }}
{{- end }}
{{- end }}
