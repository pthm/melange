...
func fullMigration(t *testing.T, schemaContent, version string) (compiledSchema, compiler.MigrationSQL) {
	...
	mig := compiler.GenerateMigrationSQL(cs.genSQL, cs.listSQL, cs.functionNames, compiler.MigrationOptions{
		Version:        version,
		SchemaChecksum: migrator.ComputeSchemaChecksum(schemaContent),
...
func incrementalMigration(t *testing.T, schemaContent, version string, prev compiledSchema) (compiledSchema, compiler.MigrationSQL) {
	...
	mig := compiler.GenerateMigrationSQL(cs.genSQL, cs.listSQL, cs.functionNames, compiler.MigrationOptions{
		...
		SchemaChecksum:        migrator.ComputeSchemaChecksum(schemaContent),
		CodegenVersion:        migrator.CodegenVersion(),
		PreviousFunctionNames: prev.functionNames,
		PreviousChecksums:     migrator.ComputeFunctionChecksums(prev.namedFunctions),
		PreviousSource:        "test",
		NamedFunctions:        cs.namedFunctions,
	})
	return cs, mig
